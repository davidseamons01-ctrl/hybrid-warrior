...
const EX=window.EX;
function exById(id){return window.exById(id)}
const EX_MEDIA=window.EX_MEDIA;
const EX_MEDIA_FEMALE=window.EX_MEDIA_FEMALE;
const EX_QUICK_DEMO_VIDEO=window.EX_QUICK_DEMO_VIDEO;
const EX_MUSCLE_IDS=window.EX_MUSCLE_IDS;
...
(async function(){
  ...
  let cfg=getResolvedFirebaseConfig();
  if(cfg){try{showAuthLoading();await initFB(cfg);fbAuth.onAuthStateChanged(u=>{if(u)enterApp(u);else{offlineMode=false;showAuthLogin()}})}catch{showAuthSetup()}}
  else showAuthSetup();
