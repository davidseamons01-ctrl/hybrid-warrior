...
function phaseName(w){try{return goalPhaseLabel(currentPlanGoal(),w);}catch(e){return w<=4?"Hypertrophy":w<=8?"Strength":w<=12?"Peak":"Test";}}
...
// Build the scoring context from the onboarding profile/goals/schedule.
function planCtx(){
  const fa=S.goals.focusAreas||[];
...
function recommendedPlans(n){
  const ctx=planCtx();
  return rankPlans(PLANS,ctx).slice(0,n||3).map(r=>({id:r.plan.id,plan:r.plan,why:r.why,score:r.score}));
}
// The active plan's goal (drives goal-aware periodization).
function currentPlanGoal(){
  const plan=PLANS[S.planId!=null?S.planId:0];
  return (plan&&plan.goal)||"hybrid";
}
// Rolling working 1RM: blend frozen profile max with recent logged best so the
// program gets smarter the more you log (clamped against single-session flukes).
function wmax1RM(type){
  try{
    const map={bench:["bench1RM","bench"],squat:["squat1RM","squat"],dead:["dead1RM","deadlift"]};
    const cfg=map[type];if(!cfg)return Number(S.profile[type+"1RM"])||0;
    const e=exById(cfg[1]);
    const rb=recentBestE1RM(S.logs||[],e?e.name:cfg[1]);
    return workingMax(Number(S.profile[cfg[0]])||0,rb);
  }catch(err){return Number(S.profile[(type==="dead"?"dead":type)+"1RM"])||0;}
}
...
function subExForEquip(ex){
  try{
    const equip=userEquip();
    const need=exerciseNeeds(ex.eid);
    const set=equipSetOf(equip);
    if(need==="bodyweight"||set.has(need))return ex;
    const newEid=substituteEid(ex.eid,equip);
    if(newEid===ex.eid)return ex;
    const ne=exById(newEid),name=ne?ne.name:newEid;
    if(exerciseNeeds(newEid)==="bodyweight"){
      const keepUnit=(newEid==="plank"||newEid==="face_pull");
      return {...ex,eid:newEid,target:keepUnit?ex.target:0,unit:keepUnit?ex.unit:"BW",reason:`No ${need} — swapped to ${name}`};
    }
    const uni=["bss","lunge","suitcase"].includes(newEid);
    const scale={incline_db:0.4,rdl:0.45,hip_thrust:0.5,bss:0.2,suitcase:0.2,cross_press:0.12,champagne:0.1}[newEid]||0.4;
    return {...ex,eid:newEid,target:r5((Number(ex.target)||0)*scale),unit:uni?"lb/hand":"lb",reason:`No ${need} — swapped to ${name}`};
  }catch(err){return ex;}
}
// Replace any hard-coded numeric warmup ramp with one scaled to the day's main
// lift. Only touches days that actually have a weighted lift, so running/
// bodyweight warmups (which may legitimately contain digits) are left intact.
function injectComputedWarmup(out){
  try{
    const weighted=(out.exs||[]).filter(e=>/lb/.test(e.unit||"")&&Number(e.target)>0);
    if(!weighted.length)return; // run/bodyweight day — leave warmup untouched
    const intro=String(out.warmup||"").split(/→\s*(?:bar|\d)/)[0].replace(/→\s*$/,"").trim();
    const ramp=warmupText(Number(weighted[0].target)||0,{unit:"lb"});
    out.warmup=ramp?(intro?intro+" → ":"")+ramp:(intro||out.warmup);
  }catch(err){}
}
// After equipment swaps, two slots can collapse to the same movement. Vary any
...
function dedupeExs(exs){
  try{
    ...
    return (exs||[]).map(ex=>{
      ...
      const pool=(tags.includes("legs")||tags.includes("lower"))?legPool:pushPool;
      const alt=pool.find(a=>!seen.has(a)&&exerciseNeeds(a)==="bodyweight");
      if(!alt){seen.add(ex.eid);return ex;}
      seen.add(alt);const ae=exById(alt);
      return {...ex,eid:alt,target:0,unit:"BW",reason:`Varied to ${ae?ae.name:alt} (avoids repeat)`};
    });
  }catch(err){return exs;}
}
// Keep an explicit choice; otherwise score the whole catalog instead of always
// returning plan 0. THIS is what makes onboarding personalized.
function selectPlan(){
  if(S.planId!=null)return S.planId;
  try{return bestPlanId(PLANS,planCtx());}catch(e){return 0;}
}
function wkFactor(w){try{return wkFactorFor(currentPlanGoal(),w);}catch(e){return[.62,.65,.68,.60,.72,.76,.79,.65,.80,.84,.88,.92,.85][clamp(w,1,13)-1];}}
function phaseReps(w){try{return phaseRepsFor(currentPlanGoal(),w);}catch(e){return w<=4?[10,8,8,6]:w<=8?[6,5,4,3]:w<=12?[4,3,2,2]:[3,2,1,1];}}
function phaseSets(w){try{return phaseSetsFor(currentPlanGoal(),w);}catch(e){return w<=4?4:w<=8?5:w<=12?5:3;}}
...
function startRestTimer(sec,nameHint,subLabel){
  ...
  if(!bar||!tim)return;
  lab.textContent=(nameHint?"Rest — "+nameHint:"Rest between sets")+(subLabel?" · "+subLabel:"");
...
// Compact "per side" plate breakdown shown directly on a barbell lift card —
// no mental math at the rack. Reuses the calculator's plate logic.
function inlinePlateMathHtml(ex){
  try{
    if(!ex||exerciseNeeds(ex.eid)!=="barbell")return"";
    if(String(ex.unit||"")!=="lb")return"";
    const tgtLb=Number(ex.target)||0;
    if(tgtLb<=0)return"";
    let parts,bar,unit;
    if(useMetric()){
      bar=20;unit="kg";
      const totalKg=Math.round((tgtLb/LB_PER_KG)/2.5)*2.5;
      const r=calcPlatesPerSideKg(totalKg,bar);
      if(r.perSide<=0)return"";
      parts=r.plates.map(x=>(x.n>1?x.n+"×":"")+x.kg).join(" + ")+(r.ok?"":" ≈");
    }else{
      bar=45;unit="lb";
      const r=calcPlatesPerSide(tgtLb,bar);
      if(r.perSide<=0)return"";
      parts=r.plates.map(x=>(x.n>1?x.n+"×":"")+x.lb).join(" + ")+(r.ok?"":" ≈");
    }
    if(!parts.trim())return"";
    return`<div class="ex-plate-inline" style="font-size:11px;color:var(--text2);margin-top:4px;display:flex;align-items:center;gap:5px;line-height:1.3"><span aria-hidden="true">🏋️</span><span>Per side: <b style="color:var(--text)">${parts}</b> <span style="color:var(--text3)">(${bar} ${unit} bar)</span></span></div>`;
  }catch(e){return"";}
}
...
function ytId(u){
  if(!u)return"";
  const m=u.match(/(?:youtube\.com\/embed\/|youtube\.com\/watch\?v=|youtube\.com\/shorts\/|youtu\.be\/)([A-Za-z0-9_-]{6,})/);
  return m?m[1]:"";
}
...
function renderLog(){
  ...
  return`<div class="section">${editBanner}<div class="card"><div class="card-h"><h2>Session report</h2><span class="badge badge-ice">Updates loads</span></div>
    ...
    ${plan.exs.length?`<div class="table-wrap"><table class="log-report-table log-report-stack"><thead><tr><th>Exercise</th><th>Target</th><th>Sets</th><th>Reps / min</th><th>Load / pace</th><th>Notes</th></tr></thead><tbody>${plan.exs.map((ex,i)=>{const e=exById(ex.eid);const nm=e?e.name:ex.eid;const run=isRunExerciseName(nm);const w0=typeof ex.target==="number"&&ex.target>0?(run?paceSecPerMiDisplay(ex.target):loadInputDisplayFromLb(ex.target)):0;const wCell=run?`<input type="text" class="input-sm input-mmss" id="lg-w${i}" value="${w0}" placeholder="8:42" inputmode="numeric" autocomplete="off" style="width:88px">`:`<input type="number" class="input-sm" id="lg-w${i}" value="${w0}" min="0" step="any" style="width:70px">`;return`<tr><td class="lr-name" style="color:var(--text);font-weight:600">${nm}</td><td data-label="Target" style="white-space:nowrap">${formatPrescribedRx(ex)}</td><td data-label="Sets"><input type="number" class="input-sm" id="lg-s${i}" value="${ex.sets}" min="1" style="width:55px"></td><td data-label="Reps / min"><input type="number" class="input-sm" id="lg-r${i}" value="${ex.reps}" min="1" style="width:55px"></td><td data-label="Load / pace">${wCell}</td><td data-label="Notes"><input type="text" class="input-sm" id="lg-n${i}" placeholder="optional" style="width:100px"></td></tr>`}).join("")}</tbody></table></div>
...
function mkDay(slot,w){
  ...
  // Build a fully personalized accessory: equipment-filtered + week-rotated
  // pick, then sets/reps/load from accessoryRx (phase + experience + the
  // user's own logged history for that movement).
  const accFrom=(pool,base,salt)=>{
    ...
    const e=exById(o.eid),name=e?e.name:o.eid;
    const weighted=o.ratio>0;
    const last=weighted?((S.lastLiftByEid&&Number(S.lastLiftByEid[o.eid]))||0):0;
    const recent=weighted?(S.logs||[]).filter(l=>l.exercise===name).slice(-1)[0]:null;
    const beatTarget=recent?((Number(recent.aR)||0)*(Number(recent.aS)||0))>((Number(recent.tR)||0)*(Number(recent.tS)||0)):false;
    const rx=accessoryRx({goal:currentPlanGoal(),week:w,isolation:!o.comp,experienceMonths:(S.profile.prefs||{}).experienceMonths,estTarget:weighted?base*o.ratio:0,lastLogged:last,lastFeel:recent?recent.liftFeel:null,beatTarget});
    let target=weighted?r5(rx.target):0;
    if(weighted&&last>0&&/progressed/.test(rx.basis)&&target<=last)target=last+5; // ensure a real bump survives rounding
    return {eid:o.eid,sets:rx.sets,reps:rx.reps,target,unit:o.unit||(weighted?"lb":"BW"),reason:`${name} — ${o.tag||"accessory"} · ${weighted?rx.basis:"bodyweight"}`};
  };
  const PUSH_ACC=[{eid:"incline_db",ratio:.4,unit:"lb/hand",tag:"upper chest",comp:1},{eid:"cgbench",ratio:.65,tag:"triceps",comp:1},{eid:"machine_chest_press",ratio:.6,tag:"chest volume",comp:1},{eid:"dip",ratio:0,tag:"triceps",comp:1},{eid:"decline_pushup",ratio:0,tag:"chest",comp:1}];
  const LEG_ACC=[{eid:"lunge",ratio:.2,unit:"lb/hand",tag:"unilateral",comp:1},{eid:"bss",ratio:.18,unit:"lb/hand",tag:"single-leg",comp:1},{eid:"step_up",ratio:.18,unit:"lb/hand",tag:"step-up",comp:1},{eid:"leg_press",ratio:1.2,tag:"quad volume",comp:1},{eid:"leg_ext",ratio:.35,tag:"quad isolation"},{eid:"leg_curl",ratio:.3,tag:"hamstring"},{eid:"glute_bridge",ratio:0,tag:"glutes"},{eid:"air_squat",ratio:0,tag:"bodyweight squat",comp:1}];
  const SH_ACC=[{eid:"ohp",ratio:.3,tag:"overhead press",comp:1},{eid:"lat_raise",ratio:.08,unit:"lb/hand",tag:"side delt"},{eid:"arnold_press",ratio:.25,unit:"lb/hand",tag:"shoulder press",comp:1},{eid:"rear_delt_fly",ratio:.06,unit:"lb/hand",tag:"rear delt"},{eid:"pike_pushup",ratio:0,tag:"bodyweight shoulders",comp:1}];
  const ARM_ACC=[{eid:"bb_curl",ratio:.3,tag:"biceps"},{eid:"db_curl",ratio:.15,unit:"lb/hand",tag:"biceps"},{eid:"hammer_curl",ratio:.15,unit:"lb/hand",tag:"biceps"},{eid:"tricep_pushdown",ratio:.3,tag:"triceps"},{eid:"skullcrusher",ratio:.3,tag:"triceps"},{eid:"overhead_ext",ratio:.2,unit:"lb/hand",tag:"triceps"},{eid:"diamond_pushup",ratio:0,tag:"bodyweight triceps"}];
  const PULL_ACC=[{eid:"pullup",ratio:0,tag:"vertical pull",comp:1},{eid:"lat_pulldown",ratio:.6,tag:"lats",comp:1},{eid:"seated_cable_row",ratio:.6,tag:"mid back",comp:1},{eid:"chest_supported_row",ratio:.3,unit:"lb/hand",tag:"upper back",comp:1},{eid:"face_pull",ratio:0,unit:"band",tag:"rear delt"},{eid:"inverted_row",ratio:0,tag:"bodyweight row",comp:1}];
  ...
  out.exs=dedupeExs((out.exs||[]).map(subExForEquip));
