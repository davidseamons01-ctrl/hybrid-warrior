...
function Stepper({ id, value, delta, min, step, label, repLab }: {
  id: string; value: string | number; delta: number; min: number; step?: string; label?: string; repLab?: string;
}) {
  return (
    <div>
      <label>{label ?? repLab}</label>
      <div class="stepper">
        <button type="button" class="step-btn" data-target={id} data-delta={-delta}>−</button>
...
function LoadCol({ id, val, wStep, unit, i }: { id: string; val: string; wStep: number; unit: string; i: number }) {
  return (
    <div>
      <label>Load ({unit})</label>
      <div class="stepper">
        <button type="button" class="step-btn" data-target={id} data-delta={-wStep}>−</button>
    ...
    <div class={"ex-card" + (p.done ? " ex-done" : "")} id={"exc-" + i} data-eid={p.eid}>
      <div class="ex-top ex-top-row">
        ...
        <div class="ex-actions ex-actions-stack">
          <div class="ex-actions-primary">
            <button type="button" class="btn btn-sm btn-secondary-solid ex-rest" data-i={i} title={`Rest timer (${p.restTitle})`}>Rest · {p.restHuman}</button>
            <button type="button" class="btn btn-sm btn-cta ex-toggle" data-i={i}>Details &amp; video</button>
          </div>
          <div class="ex-actions-secondary">
            <button type="button" class="ex-link-btn ex-skip" data-eid={p.eid} title="Remove from today's checklist">Skip</button>
      ...
      <div class="ex-body" id={"exb-" + i}>
        ...
        <div class="feel-chips">
          ...
          <button type="button" class="feel-chip" data-feel="easy" data-i={i}>Too easy (RPE &lt; 7)</button>
          <button type="button" class="feel-chip on" data-feel="ok" data-i={i}>Just right (RPE 7-8)</button>
          <button type="button" class="feel-chip" data-feel="hard" data-i={i}>Too hard (RPE 9+)</button>
        </div>
        <Html html={p.runRpeSelectHtml} />
        <div class="ex-note-wrap">
          <label class="ex-note-label">My notes for {p.exNm}</label>
          <textarea class="ex-note-input" data-eid={p.eid} placeholder="Cues, grip width, stance notes…" rows={2} maxlength={500} value={p.savedNote} />
        ...
        <div class="quick-log-row">
          ...
          <Stepper id={"tq-r" + i} value={p.reps} delta={1} min={1} repLab={p.repLab} />
          {p.runEx ? <PaceCol id={"tq-w" + i} val={p.quickWVal} /> : <LoadCol id={"tq-w" + i} val={p.quickWVal} wStep={p.wStep} unit={unit} i={i} />}
          <OutcomeSelect id={"tq-o" + i} />
          {p.runEx && p.hasShoe ? <div id={"shoe-pick-" + i}><Html html={p.shoeHtml} /></div> : null}
          <button type="button" class="btn btn-cta btn-block q-save" data-i={i}>Complete set &amp; start rest</button>
        </div>
        <details class="ex-logall-details" style="margin-top:6px">
          ...
          <div class="ex-log-grid" style="margin-top:8px">
            <Stepper id={"t-s" + i} value={p.sets} delta={1} min={1} label="Sets" />
            <Stepper id={"t-r" + i} value={p.reps} delta={1} min={1} repLab={p.repLab} />
            {p.runEx ? <PaceCol id={"t-w" + i} val={p.gridWVal} /> : <LoadCol id={"t-w" + i} val={p.gridWVal} wStep={p.wStep} unit={unit} i={i} />}
            <OutcomeSelect id={"t-o" + i} />
            <button type="button" class="btn btn-sm btn-secondary-solid ex-copyprev" data-i={i}>Copy previous set</button>
            <button type="button" class="btn btn-cta btn-sm ex-save" data-i={i}>Save all</button>
          </div>
        </details>
